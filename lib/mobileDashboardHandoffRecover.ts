 ...
 * Si cookie mobile + session Discord existent sur une page /member (dashboard, présences, etc.),
 * on renvoie vers le handoff pour émettre le code et rediriger vers tenfmobile://auth?code=…
 */
export async function redirectMemberDashboardToHandoffIfMobileContext(
  request: NextRequest
): Promise<NextResponse | null> {
  const path = request.nextUrl.pathname;
  if (path !== "/member" && !path.startsWith("/member/")) return null;
