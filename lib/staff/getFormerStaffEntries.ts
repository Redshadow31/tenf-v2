...
export async function getFormerStaffEntries(): Promise<OrgChartEntry[]> {
  try {
    const entries = await staffOrgChartRepository.listFormerStaff();
    return await enrichEntriesWithAvatars(entries);
  } catch (error) {
    console.error("[getFormerStaffEntries] Erreur:", error);
    return [];
  }
}
