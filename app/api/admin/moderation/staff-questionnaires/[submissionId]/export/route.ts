...
import { buildExportRows, getSubmissionById, rowsToCsv } from "@/lib/staff-questionnaire/storage";
...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const rows = await buildExportRows(templateId, submissionId);
    ...
    return new NextResponse(csv, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="questionnaire-staff-${slug}.csv"`,
      },
    });
  } catch (error) {
    console.error("[staff-questionnaires submission export]", error);
