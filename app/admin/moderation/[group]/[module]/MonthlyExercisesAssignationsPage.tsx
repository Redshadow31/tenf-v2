...
export default function MonthlyExercisesAssignationsPage() {
  ...
  return (
    <div className="min-h-screen space-y-5 bg-[#0b0f1a] p-6 text-white md:p-8">
      <section className="rounded-2xl border border-[#353a50] bg-[linear-gradient(145deg,rgba(67,56,202,0.16),rgba(12,15,24,0.94)_48%,rgba(16,185,129,0.12))] p-5">
        <p className="text-xs uppercase tracking-[0.12em] text-indigo-200">Admin / Modération / Petits travaux</p>
        <h1 className="mt-2 text-2xl font-semibold">Assignations exercices mensuels</h1>
        <p className="mt-2 text-sm text-slate-300">
          Génère la campagne, sélectionne les modérateurs cibles puis verrouille l&apos;envoi du mois.
        </p>
      </section>
      ...
      <section className="grid grid-cols-1 gap-3 rounded-2xl border border-slate-700 bg-[#101523]/85 p-4 md:grid-cols-4">
        ...
        <label className="text-sm">
          <span className="mb-1 block text-xs uppercase tracking-[0.1em] text-slate-400">Exercices par modérateur</span>
          <input
            type="number"
            min={5}
            max={templateCount}
            value={count}
            onChange={(event) => setCount(Math.min(templateCount, Math.max(5, Number(event.target.value) || 5)))}
            className="w-full rounded-lg border border-slate-600 bg-[#0f1422] px-3 py-2 text-slate-100"
          />
        </label>
        ...
        <div className="text-sm">
          ...
          <div className="rounded-lg border border-slate-600 bg-[#0f1422] p-2 text-xs text-slate-200">
            Au moins 1 facile, 1 moyen et 1 difficile. Les exercices générés sont assignés à chaque modérateur sélectionné.
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
          {staff.map((member) => (
            <label
              ...
              className="flex items-center gap-2 rounded-lg border border-slate-700 bg-[#0f1422] px-3 py-2 text-sm text-slate-200"
            >
              <input
                type="checkbox"
      ...
      {!campaign ? (
        ...
        <section className="space-y-3">
          {campaign.exercises.map((exercise, index) => {
            const exerciseAssignments = campaign.assignments.filter((item) => item.exerciseId === exercise.id);
            const submittedCount = exerciseAssignments.filter((item) => item.status === "submitted").length;
            return (
              <article key={exercise.id} className="rounded-2xl border border-slate-700 bg-[#101523]/85 p-5">
                ...
                <p className="mt-3 text-xs text-slate-300">
                  Assigné à {exerciseAssignments.length} modérateur(s) • {submittedCount} soumission(s)
                </p>
              </article>
            );
          })}
