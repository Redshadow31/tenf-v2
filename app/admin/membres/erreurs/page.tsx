...
export default function ErreursMembresPage() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      router.replace("/admin/membres/incomplets?vue=erreurs");
    }, 300);
  ...
  return (
    <div className="min-h-screen bg-[#0e0e10] p-8 text-white">
      <div className="mx-auto max-w-3xl rounded-2xl border border-indigo-300/20 bg-[linear-gradient(150deg,rgba(99,102,241,0.12),rgba(14,15,23,0.85)_45%,rgba(56,189,248,0.08))] p-6 shadow-[0_20px_50px_rgba(2,6,23,0.45)]">
        <p className="text-xs uppercase tracking-[0.12em] text-indigo-200/90">Page fusionnée</p>
        <h1 className="mt-2 text-2xl font-semibold">Erreurs & incohérences est intégré à Profils incomplets</h1>
        <p className="mt-3 text-sm text-slate-300">
          Cette route reste active pour compatibilité des liens existants, puis redirige automatiquement vers la vue erreurs de la page fusionnée.
        </p>
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          >
            Ouvrir la vue erreurs
            <ArrowRight className="h-4 w-4" />
          </Link>
          <button
            type="button"
            onClick={() => router.replace("/admin/membres/incomplets?vue=erreurs")}
