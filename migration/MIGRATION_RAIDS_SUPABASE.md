...
**Date** : $(date)  
**Objectif** : Migrer les raids de Netlify Blobs vers Supabase et créer une fonction SQL pour calculer les stats

---

## 📋 État Actuel

Les raids sont actuellement stockés dans **Netlify Blobs** avec la structure suivante :
...
### Étape 1 : Créer la Table `raids` dans Supabase

```sql
-- Table pour stocker les raids individuels
...
### Étape 2 : Créer la Fonction SQL `compute_raid_stats()`

```sql
-- Fonction pour calculer les statistiques de raids
...
RETURNS TABLE (
  ...
  targets JSONB -- Détail des raids vers chaque cible
) AS $$
DECLARE
  v_month_key TEXT;
BEGIN
...
### Étape 3 : Fonction SQL pour les Stats Globales (ComputedRaidStats)

```sql
-- Fonction pour calculer les statistiques globales (équivalent à ComputedRaidStats)
...
### Dans TypeScript (Repository)

```typescript
// lib/repositories/RaidRepository.ts
import { supabaseAdmin } from '@/lib/db/supabase';

export class RaidRepository {
  async getRaidStats(monthKey?: string): Promise<RaidStats[]> {
  ...
  async getGlobalRaidStats(monthKey?: string): Promise<ComputedRaidStats> {
    const { data, error } = await supabaseAdmin.rpc('compute_raid_stats_global', {
      p_month_key: monthKey || null
    });
...
**Les raids doivent d'abord être migrés depuis Netlify Blobs vers Supabase avant d'utiliser ces fonctions SQL.**

Souhaitez-vous que je crée :
1. ✅ Le script de migration des données depuis Netlify Blobs vers Supabase ?
2. ✅ Le repository `RaidRepository` pour utiliser ces fonctions SQL ?
