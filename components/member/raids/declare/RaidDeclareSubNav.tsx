...
import { ClipboardList, Heart, History, Shield } from "lucide-react";
...
const ITEMS = [
  { id: "declare-form", label: "Formulaire", icon: ClipboardList },
  { id: "declare-dossiers", label: "Mes dossiers", icon: History },
  { id: "declare-why", label: "Repères", icon: Heart },
] as const;

export default function RaidDeclareSubNav() {
  ...
  return (
    <nav
      aria-label="Sections déclaration raid"
    ...
    >
      <div className="flex flex-nowrap items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <span
          className="ml-auto hidden shrink-0 items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold text-violet-100/90 sm:inline-flex"
          style={{
            borderColor: hexToRgba(RAID_DECLARE_ACCENT, 0.35),
            background: hexToRgba(RAID_DECLARE_ACCENT, 0.12),
          }}
        >
